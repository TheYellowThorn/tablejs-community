import { AfterViewInit, Directive, ChangeDetectorRef, ContentChild,
  ElementRef, EmbeddedViewRef, EventEmitter, Inject, Input, OnInit, Output, TemplateRef, ViewRef, OnDestroy, AfterViewChecked, AfterContentChecked, DoCheck, Renderer2} from '@angular/core';
...
export class ScrollViewportDirective implements AfterContentChecked, AfterViewChecked, AfterViewInit, DoCheck, OnDestroy, OnInit {
  ...
  private addScrollItems(index: number, overflow: number): void {
    ...
    for (let i = this.adjustedStartIndex!; i < this.adjustedStartIndex! + this.itemLoadLimit; i++) {
      if (i < 0) {
        continue;
      }
  ...
  public scrollToExact(index: number, overflow: number = 0): void {
    ...
    this.adjustedStartIndex = index - this.preItemOverflow <= 0 ? 0 : index - this.preItemOverflow;
  ...
  private getRangeChange(scrollChange: number) {
    ...
    if (scrollChange > 0) {
      for (let i = this.range.startIndex; i! <= this.range.endIndex! + this.itemLoadLimit; i!++) {
