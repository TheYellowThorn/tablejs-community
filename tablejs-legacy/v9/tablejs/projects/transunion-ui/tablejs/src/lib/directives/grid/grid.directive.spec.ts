...
import { AfterViewInit, Component, ComponentFactory, ComponentFactoryResolver, ComponentRef, Directive, ElementRef, EmbeddedViewRef, Injector, Input, NgModuleRef, TemplateRef, ViewContainerRef, ViewRef } from '@angular/core';
...
describe('GridDirective', () => {
  ...
  beforeEach(() => {
    ...
    directive.scrollViewportDirective = new MockScrollViewportDirective(
      new MockElementRef(),
      directive['gridService'],
      directive['document'],
      directive['directiveRegistrationService'],
  ...
  describe('registerDirectiveToElement()', () => {
    it('should add grid directive to the nativeElement', () => {
      directive['registerDirectiveToElement']();
      expect(directive['elementRef'].nativeElement.gridDirective).toBe(directive);
    });
    it('should add the grid directive to the nativeElement\'s parent', () => {
      directive['registerDirectiveToElement']();
  ...
  describe('attachMutationObserver()', () => {
    it('should set the observer', () => {
      directive['attachMutationObserver']();
  ...
  describe('getChildNodes()', () => {
    it('should register the child node', () => {
      const spy = spyOn(directive['directiveRegistrationService'], 'registerNodeAttributes');
      const div = document.createElement('div');
      const div2 = document.createElement('div');
      div.appendChild(div2);
      directive['getChildNodes'](div);
      expect(spy).toHaveBeenCalledWith(div2);
    });
    it('should call getChildNodes again if the child is an HTMLElement', () => {
      const spy = spyOn<any>(directive, 'getChildNodes').and.callThrough();
      const div = document.createElement('div');
      const div2 = document.createElement('div');
      div.appendChild(div2);
    ...
    it('should not call getChildNodes again if the child is not an HTMLElement', () => {
      const spy = spyOn<any>(directive, 'getChildNodes').and.callThrough();
      const div = document.createElement('div');
      div.innerText = 'boo';
      directive['getChildNodes'](div);
  ...
  describe('ngAfterViewInit()', () => {
    it('should create and append headStyle', () => {
    ...
    describe('viewport exists', () => {
      it('should create a new scrollViewportDirective', () => {
      ...
      it('should not create a new scrollViewportDirective', () => {
      ...
      it('should not create a new scrollViewportDirective', () => {
        const viewport = directive['elementRef'].nativeElement.querySelector('*[tablejsScrollViewport');
        viewport.scrollViewportDirective = directive.scrollViewportDirective;
        const viewportRef: ElementRef = new ElementRef(viewport);
        directive.ngAfterViewInit();
        expect(viewport).toBeTruthy();
    ...
    it('should set the elementRef.native element directive', () => {
      directive.ngAfterViewInit();
      expect(directive['elementRef'].nativeElement.directive).toBe(directive);
    });
    it('should add pointer down listener', () => {
      document['hasPointerDownListener'] = false;
      const spy = spyOn(document, 'addEventListener');
      directive.ngAfterViewInit();
      expect(spy).toHaveBeenCalled();
    });
    it('should not add pointer down listener if the listener exists', () => {
      document['hasPointerDownListener'] = true;
      const spy = spyOn(document, 'addEventListener');
      directive.ngAfterViewInit();
      expect(spy).not.toHaveBeenCalled();
    });
    it('should add window.requestAnimationFrame', () => {
    ...
    it('should trigger onPointerDown', () => {
      ...
      Object.defineProperty(event, 'target', {writable: false, value: directive['elementRef'].nativeElement});
      directive['elementRef'].nativeElement.setAttribute('tablejsGrid', 'boo');
      const spy = spyOn<any>(directive, 'onPointerDown');
      directive.ngAfterViewInit();
      document.dispatchEvent(event);
      expect(spy).toHaveBeenCalled();
    });
    it('should trigger onPointerDown if parent has tablejsGrid', () => {
      const event = new PointerEvent('pointerdown');
      Object.defineProperty(event, 'target', {writable: false, value: directive['elementRef'].nativeElement});
      directive['elementRef'].nativeElement.removeAttribute('tablejsGrid');
      directive['elementRef'].nativeElement.parentElement.setAttribute('tablejsGrid', 'boo');
    ...
    // it('should not trigger onPointerDown if parent has tablejsGrid', () => {
    //   const event = new PointerEvent('pointerdown');
    //   Object.defineProperty(event, 'target', {writable: false, value: directive['elementRef'].nativeElement});
    //   directive['elementRef'].nativeElement.removeAttribute('tablejsGrid');
    //   const spy = spyOn<any>(directive, 'onPointerDown');
    ...
    //   expect(spy).not.toHaveBeenCalled();
    // });
    it('should not trigger onPointerDown', () => {
      const event = new PointerEvent('pointerdown');
  ...
  describe('onEnterFrame()', () => {
    it('should disconnect observer if columnsWithDataClasses exist', () => {
      const spy = spyOn<any>(directive.observer, 'disconnect');
      const div = document.createElement('div');
      div['dataClasses'] = ['foo'];
      directive.parentGroups = [[]];
      directive.columnsWithDataClasses = [div];
      directive['onEnterFrame'](directive, Date.now());
      expect(spy).toHaveBeenCalled();
    });
    it('should not disconnect observer if columnsWithDataClasses exist', () => {
      const spy = spyOn<any>(directive.observer, 'disconnect');
      const div = document.createElement('div');
      div['dataClasses'] = ['foo'];
      directive.parentGroups = [[]];
    ...
    it('should request another frame if no columnsWithDataClasses and mutationColumnsWithDataClasses exist', () => {
      const spy = spyOn(window, 'requestAnimationFrame');
      directive.columnsWithDataClasses = [];
    ...
    it('should set columnsWithDataClasses to mutationColumnsWithDataClasses if columnsWithDataClasses does not exist', () => {
      const spy = spyOn<any>(directive.observer, 'disconnect');
      const div = document.createElement('div');
      div['dataClasses'] = ['foo'];
      directive.parentGroups = [[]];
